# Jekyll Export Generator Plugin
...
1. Add `gem 'jekyll-export'` to your site's Gemfile and run `bundle`
...
`site.collections`, be sure to require `jekyll-export` either *after*
...
The GitHub Pages gem ignores all plugins included in the Gemfile. If you only include `jekyll-export` in the Gemfile without also including it in the `_config.yml` *the plugin will not work*. This can be confusing because the official Jekyll docs state that plugins can be included in either the Gemfile or `_config.yml`.

When building a site that uses the GitHub Pages gem, follow the instructions above and ensure that `jekyll-export` is listed in the `plugins` array in `_config.yml`.

:warning: If you are using Jekyll < 3.5.0 use the `gems` key instead of `plugins`.

## `<lastmod>` tag
The `<lastmod>` tag in the `export.xml` will reflect by priority:
...
If you would like to exclude specific pages/posts from the export set the
export flag to `false` in the front matter for the page/post.

```yml
export: false
```

To exclude files from your export. It can be achieved with configuration using [Jekyll v3.7.2 and jekyll-export v1.2.0](https://github.com/jekyll/jekyll/commit/776433109b96cb644938ffbf9caf4923bdde4d7f).
...
1. If the `export.xml` doesn't generate in the `_site` folder, ensure `_config.yml` doesn't have `safe: true`. That prevents all plugins from working.
2. If the `export.xml` doesn't generate in the `_site` folder, ensure that you don't have a sitemap generator plugin in your `_plugin` folder.
